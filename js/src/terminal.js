...
function Path() {
  ...
  Path.prototype.getFilenamePart = function(filepath, assumeFile) {
    ...
    var name = filepath
      ...
      .pop()
      .split("/")
      .pop(); // Get the final part (file name).
    if (name == "") {
      return false;
    }
    return name;
  ...
  Path.prototype.parseDirPathToSelector = function(path) {
    ...
    var name = path
      ...
      .pop()
      .split("/")
      .pop(); // Get the last part - either a file name or a dir name without a trailing slash.
...
var Terminal = (function() {
  ...
  var runCommand = function(cmd, args) {
    let toStdout = self.commands[cmd].exe(args);
    return toStdout;
  };

  var displayStdout = function(terminal, runCommandResult) {
    terminal.innerHTML += "<div>" + runCommandResult + "</div>";
  ...
  self.deleteFile = function(fileName) {
    ...
    var fileFound = currentDirChildren.querySelector(
      "f[path='" + self.path + "'][name='" + fileName + "']"
    );
  ...
  self.init = function(elem, commands, customPrompt) {
    ...
    elem.addEventListener("keypress", function(event) {
      ...
      resetPrompt(
        elem,
        prompt,
        ["clear", "reboot"].indexOf(input[0].toLowerCase()) >= 0
      );
      event.preventDefault();
    });
  ...
  self.initSession = function() {
    ...
    var fileSystemStr = localStorage.getItem("filesystem")
      ? localStorage.getItem("filesystem")
      : originalFilesystem;
    self.filesystem = new DOMParser().parseFromString(
      fileSystemStr,
      "text/xml"
    );
